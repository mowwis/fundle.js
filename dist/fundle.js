var Fundle = (() => {
          ...
          if (eventMatch) {
            ...
            if (targetElements.length > 0) {
              targetElements.forEach((targetElement) => {
                if (!targetElement._eventListeners) targetElement._eventListeners = /* @__PURE__ */ new Set();
  ...
  var Model = class extends EventTarget {
    ...
    fieldChanges = {};
    get changeDelta() {
      return Object.entries(this.fieldChanges).reduce((acc, [k, v]) => {
        acc[k] = v.newValue;
        return acc;
      }, {});
    }
    constructor(data = {}) {
      super();
      if (this.constructor._instanceCache === void 0) this.constructor._instanceCache = /* @__PURE__ */ new Map();
      if (this.constructor._classEventTarget === void 0) this.constructor._classEventTarget = new EventTarget();
    ...
    static async all(params = {}) {
      let data;
      if (this._instanceCache === void 0) this._instanceCache = /* @__PURE__ */ new Map();
    ...
    get primaryKey() {
      return this[this.constructor.primaryField];
    }
    ...
    static dispatchEvent(event) {
      if (this._classEventTarget === void 0) this._classEventTarget = new EventTarget();
      this._classEventTarget.dispatchEvent(event);
    }
    static addEventListener(type, callback) {
      if (this._classEventTarget === void 0) this._classEventTarget = new EventTarget();
      this._classEventTarget.addEventListener(type, callback);
    }
    static removeEventListener(type, callback) {
      if (this._classEventTarget === void 0) this._classEventTarget = new EventTarget();
      this._classEventTarget.removeEventListener(type, callback);
    }
    ...
    toJSON() {
      ...
      Object.keys(this.constructor.fields).forEach((field) => {
        ...
        if (value !== null && value !== void 0) jsonObject[field] = value instanceof Date ? value.toISOString() : value;
      });
      return jsonObject;
    }
  };
  var ModelEvent = class _ModelEvent extends CustomEvent {
    static NEW = "new";
    static CHANGE = "change";
    static DELETE = "delete";
    constructor(type, detail) {
      const validEvents = [_ModelEvent.NEW, _ModelEvent.CHANGE, _ModelEvent.DELETE];
    ...
    static new(target) {
      return new _ModelEvent(_ModelEvent.NEW, target);
    }
    static delete(target) {
      return new _ModelEvent(_ModelEvent.DELETE, target);
    }
    static change(target, delta) {
      return new _ModelEvent(_ModelEvent.CHANGE, { target, delta });
    }
  };
  ...
  var Component = class extends HTMLElement {
    ...
    render() {
      Templater.bind(this, this.bindings);
    }
    connectedCallback() {
  ...
  // src/collection.js
  var Collection = class _Collection extends EventTarget {
    #items = [];
    ...
    constructor(modelClass, ...initial) {
      ...
      modelClass.addEventListener("new", (e) => this.push(e.detail));
    }
    fromArray(array) {
    ...
    map(callbackfn) {
      return new _Collection(this.modelClass).fromArray(this.#items.map(callbackfn));
    }
    filter(predicate) {
      return new _Collection(this.modelClass).fromArray(this.#items.filter(predicate));
    }
    splice(start, deleteCount) {
      return new _Collection(this.modelClass).fromArray(this.#items.splice(start, deleteCount));
    }
    flatMap(callback) {
      return new _Collection(this.modelClass).fromArray(this.#items.flatMap(callback));
    }
  };
